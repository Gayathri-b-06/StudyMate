...
def record_weak_topic(
    ...
    crud.save_user_memory(
        db, user_id, MemoryFactType.WEAK_TOPIC, topic, detail, document_id
    )
...
def get_study_progress(db: Session, user_id: str) -> dict[str, list[dict[str, str]]]:
    """Return factual study records only, with no LLM-derived conclusions."""
    ...
    return {
        "quiz_attempts": [
            {
                "topic": attempt.topic,
                "score": f"{attempt.correct_count}/{attempt.total_questions}",
