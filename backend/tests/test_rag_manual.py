...
as the LLM and a local HuggingFace model (BAAI/bge-small-en-v1.5) for embeddings.
Not part of the pytest suite - run this by hand to eyeball retrieval quality
and inspect the full trace in LangSmith.
...
NOTE: Embeddings now run locally via BAAI/bge-small-en-v1.5 (sentence-transformers).
No NVIDIA_API_KEY is required. The model is auto-downloaded on first run and
cached locally. Any existing index built with a previous NVIDIA embedding model
is INCOMPATIBLE — delete the --index-path directory before your first run.
"""
...
# Embeddings must be instantiated exactly once. HuggingFaceEmbeddings loads
# the model into CPU memory on first call — reuse this singleton throughout.
# normalize_embeddings=True is required for cosine similarity with FAISS.
